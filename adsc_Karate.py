...
if __name__ == "__main__":
    ...
    com_learner = Community2Vec(model, reg_covar=reg_covar, lr=lr)
    ...
    for it in range(1):
        ...
        com_learner.train(G.nodes(), model, beta, chunksize=20, iter=5)
        log.info('time: %.2fs' % (timeit.default_timer() - start_time))
        log.info(model.centroid)

        io_utils.save_embedding(model.node_embedding, "{}_alpha-{}_beta-{}_ws-{}_neg-{}_ds-{}_lr-{}".format(output_file,
                                                                                                            alpha,
                                                                                                            beta,
                                                                                                            window_size,
                                                                                                            negative,
